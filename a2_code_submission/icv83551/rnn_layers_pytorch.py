...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h_list =[]
    h_temp = h0
    for idx in range(x_reshaped.shape[0]):
        h_temp = rnn_step_forward(x_reshaped[idx],h_temp,Wx,Wh,b)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    curr = x@Wx + prev_h@Wh + b
    i, f, o, g = torch.chunk(curr, 4, dim=1)
    g = torch.tanh(g)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    h_list =[]
    h_temp = h0
    c_temp = torch.zeros_like(h0)
    for idx in range(x_reshaped.shape[0]):
        h_temp, c_temp = lstm_step_forward(x_reshaped[idx],h_temp,c_temp,Wx,Wh,b)
        h_list.append(h_temp)
    h =torch.stack(h_list, dim=1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
