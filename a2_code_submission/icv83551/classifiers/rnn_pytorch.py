...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        words=word_embedding_forward(captions_in,W_embed)
        if self.cell_type == 'rnn':
            fr_o=rnn_forward(words, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
            fr_o=lstm_forward(words, h0, Wx, Wh, b)
        out_f =temporal_affine_forward(fr_o, W_vocab, b_vocab)
        loss = temporal_softmax_loss(out_f,captions_out,mask)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        c_prev = torch.zeros_like(h_prev) if self.cell_type == 'lstm' else None
        current_word_indices = torch.full((N,), self._start, dtype=torch.long, device=features.device) 
        for t in range(max_length):
            word=word_embedding_forward(current_word_indices, W_embed)
            if self.cell_type == 'rnn':
                h_prev = rnn_step_forward(word, h_prev, Wx, Wh, b)
            else:
                h_prev, c_prev = lstm_step_forward(word_embed, h_prev, c_prev, Wx, Wh, b)
            probs=affine_forward(h_prev, W_vocab, b_vocab )
            predict_w = torch.argmax(probs, dim=1)
            current_word_indices= predict_w
            captions[:,t]
